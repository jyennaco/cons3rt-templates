...
if [ -z "${tenantId}" ]; then
    read -p 'Tenant ID (type "skip" if not using multi-tenant login):' tenantId
fi
...
if [[ ${tenantId} == "skip" ]]; then
    echo "Logging in to Azure (follow instructions)..."
    az login
else
    echo "Logging in to Azure using the provided tenant ID (follow instructions)..."
...
if [ -z "${userPrincipalName}" ]; then
    echo "Service principal name not found, assuming AD user with proper permissions is logged in..."
else
